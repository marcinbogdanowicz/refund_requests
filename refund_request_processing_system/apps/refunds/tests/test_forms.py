...
class RefundRequestFormTests(FlushRedisDBTestMixin, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.base_data_form = base_data | {
            'reason_choice': RefundReason.WRONG_PRODUCT
        }

    def test_initial_data_new_user(self):
        form = RefundRequestForm(
            initial=RefundRequestForm.initial_for_user(self.user)
        )
    ...
    def test_reason_choice_from_standard_reason(self):
        ...
        self.assertEqual(
            form['reason_choice'].initial, RefundReason.WRONG_PRODUCT
        )
    ...
    def test_reason_choice_from_custom_reason(self):
        refund_request = RefundRequest.objects.create(
            **self.base_data_model
            | {'user': self.user, 'reason': 'Custom reason text'}
        )
